...
- [[Vagrant]] генерирует [[Inventory (Инвентарь) в Ansible|инвентарь]] (в нем прописывает [[IP-адрес]] новой ВМ и пути к [[SSH (Secure Shell)|SSH]]-ключам), после чего локально на хосте запускает `ansible-playbook` (про плейбук [[Playbook (Плейбук, Сценарий) в Ansible|здесь]]).